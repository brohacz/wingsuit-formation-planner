...
**Dive playback**: the ▶ Play action in the points bar (`startPlay`/`stopPlay`, timer in `_playT`, `PLAY_MS` = 4200 ms per point) rewinds to point 1 and auto-advances through the plan **in slow motion** — `switchPoint`'s `fromPlay` flag makes `animatePointSwitch` use the slow profile (2800 ms flight, gentle `cubic-bezier(.45,0,.25,1)` in-out curve) instead of the snappy 340 ms editing glide — stopping on the last point; while running, the button reads ◼ Stop. Playback is interrupted by anything the user does that would fight it: a manual point switch (every `switchPoint` call without the internal `fromPlay` flag stops the run — this covers chip clicks and add point), the direct `stopPlay()` calls in `delPoint`/`renamePoint`/`movePoint` (which mutate the plan without switching), opening the pilot modal, starting a mouse or touch drag, loading a plan (`fromJSON`), or pressing Esc. The Play button is disabled for single-point plans.
