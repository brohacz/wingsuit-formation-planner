    ...
    document.querySelectorAll('#fw .slot.filled, #bench-list .bench-item').forEach((el) => {
        ...
        if (old && old.bench === onBench) {
            ...
            el.animate(
                ...
                { duration: Math.round(dur * 0.76), delay: Math.round(dur * 0.24), easing: ease, fill: 'backwards' }
            );
        }
    });
}
...
function renamePoint() {
    ...
    if (n?.trim()) S.name = n.trim().slice(0, 24);
    render();
}
...
function isBench(k) {
    return typeof k === 'string' && k.startsWith('b:');
}
function getPilot(k) {
    return isBench(k) ? cur().bench[+k.slice(2)] : cur().cells[k];
}
...
function safeColor(c) {
    return typeof c === 'string' && SAFE_COLOR.test(c.trim()) ? c.trim() : '#1d9e75';
}
...
function slotHTML(k, d, i, style, compact) {
    ...
    return (
        `<button type="button" class="${cls}" data-key="${k}"${d ? ' draggable="true"' : ''} ` +
...
function slotClick(el, e) {
    ...
    if ((e.metaKey || e.ctrlKey) && cur().cells[k]) {
        toggleSelect(k);
        return;
    }
    openModal(k);
...
function dragEndHandler() {
    ...
    _drag = null;
    // a click synthesized right after the drop (if the browser fires one) is
    // dispatched before timers run, so it is still swallowed; clear the flag
    // afterwards so it can't eat an unrelated later click
    setTimeout(() => {
        _dragMoved = false;
    }, 0);
}
...
function touchReset() {
    ...
    if (_touch.active) {
        ...
        _drag = null;
        _touch.active = false;
        // touchend is preventDefault'ed mid-drag, so no synthetic click can
        // follow — safe to clear immediately instead of leaving it to eat
        // the next tap
        _dragMoved = false;
    }
}
...
document.addEventListener(
    ...
    (e) => {
        ...
        if (tgt?.el && !(tgt.kind === 'slot' && tgt.k === _drag.src)) {
...
document.addEventListener(
    ...
    (e) => {
        ...
        if (tgt) {
            if (tgt.kind === 'slot') {
                if (tgt.k !== _drag.src) dropOnSlot(_drag.src, tgt.k);
...
function dropMultiOnSlot(multi, dstK) {
    ...
    for (const m of moves) {
        if (!isValidSlot(m.to)) {
            toast('Move blocked — out of bounds');
            return;
        }
        if (targets.has(m.to)) {
            toast('Move blocked — collision');
            return;
        }
        targets.add(m.to);
        const existing = getPilot(m.to);
        if (existing && !sources.has(m.to)) {
            toast('Move blocked — collision');
            return;
        }
    }
...
function dropOnSlot(srcK, dstK) {
    if (_drag?.multi) {
        dropMultiOnSlot(_drag.multi, dstK);
        return;
    }
    const src = getPilot(srcK),
        dst = getPilot(dstK);
    if (!src) return;
    if (dst) setPilot(srcK, dst);
    else removePilot(srcK);
    setPilot(dstK, src);
    // a selected pilot dragged solo keeps their selection at the new slot
    // (bench sources are never in _selected, so delete() is a no-op there)
    if (_selected.delete(srcK)) _selected.add(dstK);
    _dragMoved = true;
    _suppressAnim = true;
...
function multiKeys(multi) {
    return [multi.leadKey, ...multi.offsets.map((o) => o.key)];
}
...
function makeBenchItem(d, i) {
    ...
    el.setAttribute(
        'aria-label',
        `Bench pilot ${d.name || 'unnamed'}, ${d.color} suit, drag to formation or click to edit`
    );
    ...
    for (let r = 0; r < ff.rows; r++) {
        for (let hx = 0; hx < hxMax; hx++) {
            ...
            html += slotHTML(
                ...
                d,
                d ? ++fi : 0,
                `position:absolute;left:${Math.round(cx - sw / 2)}px;top:${Math.round(cy - sh / 2)}px;width:${sw}px;height:${sh}px`,
                !d
            );
        }
    }
...
function renderSwatches() {
    const ctr = $('swatches');
    ctr.innerHTML = `${PALETTE.map(
        (c) => `<button type="button" class="sw" data-c="${c}" style="--sw:${c}" aria-label="Color ${c}"></button>`
    ).join('')}<input type="hidden" id="pcol" value="#1d9e75">`;
...
function normalizeForm(m, fallbackDims, withCols) {
    ...
    const out = {
        rows: clampDim(m.rows, fallbackDims?.rows),
        cells,
        bench: Array.isArray(m.bench)
            ? m.bench
                  .filter((p) => p && typeof p.name === 'string' && typeof p.color === 'string')
                  .map((p) => ({ name: p.name, color: safeColor(p.color) }))
            : [],
    };
    if (withCols) out.cols = clampDim(m.cols, fallbackDims?.cols);
    return out;
}
...
function migratePoint(d, name) {
    ...
    if (d.regular || d.diamond || d.freeform) {
        ...
        ff = d.freeform
            ? normalizeForm(d.freeform, { rows: 5, cols: 5 }, true)
...
function fromJSON(str) {
    ...
    // parse everything before committing any state, so a failed import leaves
    // the current plan (and the autofit flag) untouched. fitGrid inside
    // parsePoint reads AF, so it is set for the parse and rolled back on throw.
    const prevAF = AF;
    AF = !!d.autofit;
    let pts, pi;
    try {
...
$('export-copy').addEventListener('click', () => {
    ...
    const fallback = () => {
        // the textarea is already selected, so the legacy path can copy it
        toast(document.execCommand('copy') ? 'Copied to clipboard' : 'Copy failed — press Ctrl+C');
    };
    try {
        navigator.clipboard.writeText($('export-area').value).then(done, fallback);
    } catch (e) {
        fallback();
    }
});
...
$('import-sample').addEventListener('click', async () => {
    ...
    try {
        ...
        $('import-err').textContent =
            'Could not fetch sample-formation.json — serve the directory (e.g. python3 -m http.server) instead of opening via file://';
    }
});
...
function autosave() {
    try {
        localStorage.setItem(AUTOSAVE_KEY, toJSON());
    } catch (e) {}
}
...
function rosterList() {
    ...
    const add = (p, loc) => {
        ...
        let e = map.get(lo);
        if (!e) {
            e = { name: p.name, color: p.color, locs: [] };
            map.set(lo, e);
        }
        e.locs.push(loc);
    };
...
function rosterRemove(old) {
    const lo = old.toLowerCase().trim();
    for (const pt of PTS) {
        for (const k in pt.cells) if (pt.cells[k].name.toLowerCase().trim() === lo) delete pt.cells[k];
...
function setupRubberBand() {
    ...
    fw.addEventListener('mousedown', (e) => {
        ...
        rb = {
            ...
            el: null,
            additive: e.shiftKey,
            baseline: e.shiftKey ? new Set(_selected) : null,
        };
        e.preventDefault();
    });
    document.addEventListener('mousemove', (e) => {
        ...
        const dx = Math.abs(e.clientX - rb.startX),
            dy = Math.abs(e.clientY - rb.startY);
        ...
        const x1 = Math.min(rb.startX, e.clientX),
            y1 = Math.min(rb.startY, e.clientY);
        const x2 = Math.max(rb.startX, e.clientX),
            y2 = Math.max(rb.startY, e.clientY);
